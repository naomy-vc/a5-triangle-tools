...
public final class Encoder implements ActualParameterVisitor<Frame, Integer>,
	...
	@Override
	public Void visitDoubleCommand(DoubleCommand ast, Frame frame) {
		var pos = ast.getPosition();
		var left = new VnameExpression(ast.vname, pos);
		var lit2 = new IntegerLiteral("2", pos);
		var right = new IntegerExpression(lit2, pos);
		var op = new Operator("*", pos);
		var multiplied = new BinaryExpression(left, op, right, pos);
		var fakeAssign = new AssignCommand(ast.vname, multiplied, pos);

		visitAssignCommand(fakeAssign, frame);
	...
	public Void visitLoopWhileCommand(LoopWhileCommand ast, Frame frame) {
    	...
    	WhileCommand fakeWhile =
            new WhileCommand(ast.E, ast.C2, ast.getPosition());

    	fakeWhile.visit(this, frame);
